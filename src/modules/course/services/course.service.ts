...
export class CourseService {
    ...
    async getCourseData(user_id: number, course_id: number): Promise<FindIndividualCourseResponseDTO | CourseNotFoundException> {
        const course = await this.courseRepository.findOne({ where: { id_course: course_id } });

        if (!course) throw new CourseNotFoundException();

        const userConcluded = await this.userCourseConcludedRepository.findOne({ where: { user_id: user_id, course_id: course_id } });

        const activities = (await this.activitiesRepository.find({ where: { course_id: course_id } })).map(activity => {
            return {
